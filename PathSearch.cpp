...
void displayPathAndCost(string source,string destination,unordered_map<string,int> &dist,unordered_map<string,string> &path){
    ...
    cout << "Distance:"<< dist[destination] <<endl <<"Path:"<<endl;
    int len = p.size();
    for(int i = 1;i<len;i++){
...
void bfs(unordered_map<string,vector<pair<string,int>>> &adj,string source,string destination){
...
void ucs(unordered_map<string,vector<pair<string,int>>> &adj,string source,string destination){
...
void astar(string source, string destination,unordered_map<string,vector<pair<string,int>>> &adj,unordered_map<string, int> &heuristic) {
...
int main(int args,char* argv[]){
    ...
    string destination = argv[4];
     ...
     if(searchAlgo == "ucs"){
        ...
        astar(source, destination,adj,heuristic);
    }
    else{
